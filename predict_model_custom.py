...
if __name__ == '__main__':
    ...
    # ⬇⬇ 추가: 이미지 저장 여부 플래그 (기본값 False → 저장 안 함)
    parser.add_argument("--save-images", action="store_true", help="결과 이미지를 저장하려면 사용하세요.")
    ...
    # ⬇⬇ 이미지 저장 루프를 옵션으로 전환 (기본: 실행 안 함)
    if args.save_images:
        for i in range(len(image_list)):
            image_path = image_list[i]
            file_name = os.path.basename(image_path).split('.')[0]

            # 주의: image_arrays가 0~1 범위라면 곱하기 255 후 타입 변환 필요
            img_uint8 = (image_arrays[i] * 255.0).astype(np.uint8)
            cv2.imwrite(f"{save_dir}/seg_result_img/image/{file_name}.png",
